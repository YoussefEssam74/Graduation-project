...
namespace Service.Services
{
    public class StatsService : IStatsService
    {
        ...
        public async Task<MemberStatsDto> GetMemberStatsAsync(int memberId)
        {
            ...
            var latestBmi = latestMeasurement != null? (decimal)((double)latestMeasurement.Weight / Math.Pow((double)latestMeasurement.Height / 100.0, 2)) : (decimal?)null;
             latestBmi = (latestMeasurement != null && latestMeasurement.Height.HasValue && latestMeasurement.Height.Value > 0)
        ...
        {
            var members = await _unitOfWork.Repository<User>().GetAllAsync();
            var bookings = await _unitOfWork.Repository<Booking>().GetAllAsync();
            ...
            var totalMembers = members.Count(m => m.Role == UserRole.Member);
